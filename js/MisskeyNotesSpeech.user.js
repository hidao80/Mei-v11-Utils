...
'use strict';
async function MisskeyNotesSpeech(isRun) {
    if (!isRun) return;
    ...
    const SCRIPT_NAME = 'MisskeyNotesSpeech';
    ...
    const timer = setInterval(v => {
        const targetLane = $$one(".transition.notes") ?? $$one(".transition");
