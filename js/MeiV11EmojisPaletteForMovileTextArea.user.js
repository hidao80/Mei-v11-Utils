...
'use strict';
async function MeiV11EmojisPaletteForMovileTextArea(isRun) {
    if (!isRun) return;
    ...
    const SCRIPT_NAME = 'MeiV11EmojisPaletteForMovileTextArea';
    ...
    function main() {
        ...
        if ($$one('div.palette')?.dataset.use) {
            $$one('body>.mk-autocomplete')?.remove();
            $$onr('div.palette').dataset.use = "";
        }
        ...
        if ($$one("body div.modal div.main>div.gafaadew")) {
            console.debug("Open Post Dialog.");
            createEmojiPalette();
        }
        $$one('textarea')?.classList.add("noto-color-emoji");
    }
    function switchingTabs(e) {
        [...$$all(".tab")].map(v => { v.style.opacity = 0.65 });
        e.target.style.opacity = 1;
        [...$$all(".table")].map(v => { v.style.display = "none" });
        $$one(".table." + e.target.dataset.category).style.display = "block";
    }
    function loadEmoji(emoji, category) {
        const table = $$one('.table.' + category);
        const span = $$new("span");
        ...
        span.addEventListener("click", () => {
            ...
            $$one('div.palette').dataset.use = true;
        });
    ...
    function createEmojiPalette() {
        ...
        $$one('body div.modal div.main>div.gafaadew').insertAdjacentElement("beforeend", palette);

        // Create an area to display emoji category selection tabs.
        const tabArea = $$new("div");
        ...
        Object.keys(emojis).map(category => {
            ...
            $$one(`.tab[data-category=${category}]`).addEventListener("click", switchingTabs);
            // When loading is complete, display the palette and open the FACE page.
            $$one('.modal .main>div .palette').style.display = "block";
            $$one('.tab[data-category=face]').click();
        });
    }

    // Each time body and below are drawn, run main() to attempt to add the emoji palette to the posting area.
    new MutationObserver(main).observe($$one('body'), { childList: true });
